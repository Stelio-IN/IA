...
class State:
    ...
    def calculate_h(self):
        ...
        self.h = (people_left + 1) // 2  # Mínimo de travessias restantes
    ...
    def __str__(self):
        return (f"Left: {self.left_m}M {self.left_c}C | Boat: {self.boat} | "
                f"Right: {self.right_m}M {self.right_c}C | f={self.f}")
...
def print_solution(path):
    for i, state in enumerate(path):
        if i == 0:
            print(f"Passo {i}: Estado inicial (f={state.f})")
        else:
            print(f"Passo {i}: {state.action} (f={state.f})")
        print(state)
        print()
